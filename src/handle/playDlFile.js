...
async function playDlSong(message, songManager) {
  try {
    ...
    songManager.addSong(guildId, convertedFilePath, attachment.name, key);

    if (isIdle) {
      playNextSong(player, message, songManager);
    }
  } catch (error) {
